...
public abstract class RtcBaseActivity extends BaseActivity implements EventHandler {
    ...
    private void joinChannel() {
        ...
        String token = config().getToken(config().getChannelName());
        Log.e("config",token);
        Log.e("config",config().getUid()+"");
        Log.e("config",config().getChannelName());
        ...
        configVideo();
        rtcEngine().joinChannel(token, config().getChannelName(), "", config().getUid());
    }
