...
public class LiveActivity extends RtcBaseActivity {
    ...
    private VideoEncoderConfiguration.VideoDimensions dimensions = new VideoEncoderConfiguration.VideoDimensions(720, 1080);
...
//        joinChannel(config().getChannelName());
    ...
    private void startPublish(boolean encode) {
        if (encode) {
            ...
            localTranscodingUser.width = 360 ;
            localTranscodingUser.height = 640 ;
            ...
            for (int i = 0; i < users.size(); i++) {
                if (users.get(i) ==config().getUid()) continue;
                LiveTranscoding.TranscodingUser remoteUser = new LiveTranscoding.TranscodingUser();
                remoteUser.x = lastX+(360)>=720?0:lastX+360;
                remoteUser.y = lastX+(360)>=720?lastY+640:lastY;
                lastX = remoteUser.x;
                lastY = remoteUser.y;
                Log.e(TAG,"lastX:"+lastX);
                Log.e(TAG,"lastY:"+lastY);
                remoteUser.width = 360;
                remoteUser.height = 720;
                remoteUser.uid = users.get(i);
                transcoding.addUser(remoteUser);
            }
//            rtcEngine().setLiveTranscoding(transcoding);
            rtcEngine().stopRtmpStream(mEtUrl.getText().toString());
